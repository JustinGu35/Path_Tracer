...
void JSONReader::LoadSceneFromFile(QFile &file, const QString &local_path, Scene &scene)
{
...
bool JSONReader::LoadGeometry(QJsonObject &geometry, QMap<QString, std::shared_ptr<Material>> mtl_map, const QString &local_path, std::vector<uPtr<Primitive>> *primitives)
{
    ...
    auto primitive = mkU<Primitive>(shape);
...
bool JSONReader::LoadLights(QJsonObject &geometry, QMap<QString, std::shared_ptr<Material>> mtl_map, const QString &local_path, std::vector<std::unique_ptr<Primitive> > *primitives, std::vector<std::shared_ptr<Light> > *lights)
{
    std::shared_ptr<Shape> shape = nullptr;
    //First check what type of geometry we're supposed to load
    QString type;
    ...
    //load transform to shape
    if(geometry.contains(QString("transform")) && shape != nullptr) {
        QJsonObject transform = geometry["transform"].toObject();
        shape->transform = LoadTransform(transform);
    }
    ...
    else if (QString::compare(lgtType, QString("PointLight")) == 0)
    {
        ...
        Float intensity = static_cast< float >(geometry["intensity"].toDouble());
        QJsonObject transform = geometry["transform"].toObject();
        lightType = std::make_shared<PointLight>(LoadTransform(transform), lightColor * intensity, mScene);
    }
    else if (QString::compare(lgtType, QString("SpotLight")) == 0)
    {
        ...
        Float falloff = static_cast< float >(geometry["falloff"].toDouble());
        QJsonObject transform = geometry["transform"].toObject();
        lightType = std::make_shared<SpotLight>(LoadTransform(transform), lightColor * intensity, mScene, totalWidth, falloff);
    }
    else
    {
        std::cout << "Could not parse the geometry!" << std::endl;
    ...
    auto primitive = mkU<Primitive>(shape, nullptr,  std::static_pointer_cast<AreaLight>(lightType));
    QMap<QString, std::shared_ptr<Material>>::iterator i;
    if(geometry.contains(QString("material"))) {
        QString material_name = geometry["material"].toString();
        for (i = mtl_map.begin(); i != mtl_map.end(); ++i) {
    ...
    (*lights).push_back(lightType);
    return true;
}

bool JSONReader::LoadMaterial(QJsonObject &material, QString local_path, QMap<QString, std::shared_ptr<Material> > *mtl_map)
{
    QString type;
    ...
    if(QString::compare(type, QString("MatteMaterial")) == 0)
    {
        ...
        if(material.contains(QString("textureMap"))) {
            QString img_filepath = local_path.append(material["textureMap"].toString());
