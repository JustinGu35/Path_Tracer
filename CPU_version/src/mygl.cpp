...
#include "integrators/surfacenormalsintegrator.h"
...
MyGL::MyGL(QWidget *parent)
    : GLWidget277(parent),
      ...
      recursionLimit(5),
      integratorType(SURFACE_NORMALS),
      completeSFX(),
      is_multithreaded(false)
{
    ...
    completeSFX.setSource(QUrl::fromLocalFile(":/include/complete.wav"));
}
...
void MyGL::SceneLoadDialog()
{
    ...
    QString local_path = filepath.left(i+1);
...
inline void MyGL::setupProgressiveTexture() {
...
void MyGL::RenderScene()
{
    ...
    setupProgressiveTexture();

    // Get the bounds of the film portion we are going to render:
    ...
    // We divide the screen into n 16x16-pixel tiles in order to process them on
    // parallel threads
    ...
    for(int X = 0; X < nTiles.x; X++)
    {
        for(int Y = 0; Y < nTiles.y; Y++)
        {
            ...
            // Depending on what the user has selected in the dropdown menu,
            // use a specific type of Integrator to render the scene
            Integrator* rt = nullptr;
            switch(integratorType) {
            case SURFACE_NORMALS:
                rt = new SurfaceNormalsIntegrator(tileBounds, &scene, sampler->Clone(seed), recursionLimit);
                break;
            ...
            // Use multithreading if the user has enabled it
            // (this is incompatible with breakpoint debugging,
            // so only use it if you want a fast render).
...
void MyGL::slot_setMultithreaded() {
    is_multithreaded = true;
}

void MyGL::slot_setSinglethreaded() {
    is_multithreaded = false;
}
